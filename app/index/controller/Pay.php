...
class Pay extends IndexBaseController
{
 ...
 public function nosmse()
    ...
    {
        $parentInfo = (new SystemAdmin())->query("SELECT T2.id,T2.pid,T2.username,T2.ticheng,T2.poundage
FROM ( 
    SELECT 
        @r AS _id, 
        (SELECT @r := pid FROM ds_system_admin WHERE id = _id) AS pid, 
        @l := @l + 1 AS lvl 
    FROM 
        (SELECT @r := {$this->id}, @l := 0) vars, 
        ds_system_admin h 
    WHERE @r <> 0) T1 
JOIN ds_system_admin T2 
ON T1._id = T2.id 
ORDER BY T1.lvl asc;");
