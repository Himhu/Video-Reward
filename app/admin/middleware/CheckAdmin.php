...
class CheckAdmin
{
    ...
    public function handle(Request $request, \Closure $next)
    {
        ...
        if (!in_array($currentController, $adminConfig['no_login_controller']) &&
            ...
            empty($adminId) && $this->error('请先登录后台', [], __url("@{$adminModuleName}/login/index"));

            // 判断是否登录过期
            if ($expireTime !== true && time() > $expireTime) {
                session('admin', null);
                $this->error('登录已过期，请重新登录', [], __url("@{$adminModuleName}/login/index"));
            }
        }
