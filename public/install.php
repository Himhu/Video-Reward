...
function clearDatabase($database)
{
    try {
        // 切换到目标数据库
        Db::connect('install')->execute("USE `{$database}`");

        // 获取所有表名
        $tables = Db::connect('install')->query("SHOW TABLES");

        // 删除所有表
        foreach ($tables as $table) {
            $tableName = array_values($table)[0];
            Db::connect('install')->execute("DROP TABLE IF EXISTS `{$tableName}`");
        }
...
function install($username, $password, $config, $adminUrl)
{
    ...
    try {
        ...
        @file_put_contents(ROOT_PATH . '.env', getEnvConfig($config, $adminUrl));
        Db::commit();
    } catch (\Exception $e) {
        Db::rollback();
        return $e->getMessage();
...
function getEnvConfig($data, $adminUrl)
{
    $hostname = $data['hostname'];
    $database = $data['database'];
    $username = $data['username'];
...
<div class="content">
    <p class="desc">
        使用过程中遇到任何问题可参考
        <a href="老表只要你健康" target="_blank">文档教程</a>
        <a href="老表只要你健康">QQ交流群</a>
    </p>
    <form class="layui-form layui-form-pane" action="">
        ...
        <div class="bg">
            ...
            <div class="layui-form-item">
                ...
                <div class="layui-input-block">
                    <input class="layui-input" name="database" autocomplete="off" lay-verify="required" lay-reqtext="请输入数据库名称" placeholder="请输入数据库名称" value="">
                </div>
            </div>
            ...
            <div class="layui-form-item">
                ...
                <div class="layui-input-block">
                    <input class="layui-input" name="db_username" autocomplete="off" lay-verify="required" lay-reqtext="请输入数据库账号" placeholder="请输入数据库账号" value="">
                </div>
            </div>
