define(["jquery", "tableSelect","editor"], function ($, tableSelect , editor) {
    ...
    var admin = {
        ...
        table: {
            render: function (options) {
                ...
                options.searchCols = options.searchCols || [];
                ...
                if(options.searchCols.length == 0)
                {
                    options.searchCols = options.cols;
                }
                // 判断是否为移动端
                ...
                if (options.search === true) {
                    admin.table.renderSearch(options.searchCols, options.elem, options.id);
                }
               ...
               // console.log(options.cols);
                ...
                //监听表格selects
                admin.table.renderSelects(options.cols, options.init, options.id, options.modifyReload);
                ...
                $.each(data, function (i, v) {
                    if (v === 'refresh') {
                        ...
                        if (admin.checkAuth('import', elem)) {
                            toolbarHtml += '<button class="layui-btn layui-btn-normal layui-btn-sm" data-open="' + init.import_url + '" data-title="导入"><i class="fa fa-plus"></i> 导入</button>\n';
                        }
                    }
                    else if (v === 'buy') {
                        if (admin.checkAuth('buy', elem)) {
                            toolbarHtml += '<button class="layui-btn layui-btn-normal layui-btn-sm" data-open="' + init.buy_url + '" data-width="45%"   data-title="购买域名分流"><i class="fa fa-plus"></i> 购买域名分流</button>\n';
                        }
                    }
                    else if (v === 'piliang') {
                        if (admin.checkAuth('piliang', elem)) {
                            toolbarHtml += '<button class="layui-btn layui-btn-normal layui-btn-sm" data-checkbox = "true" data-width="45%" data-height="35%" data-open="' + init.piliang_url + '" data-title="批量发布"><i class="fa fa-plus"></i> 批量导入</button>\n';
                        }
                    }
                    else if (v === 'push_all') {
                        if (admin.checkAuth('push_all', elem)) {
                            toolbarHtml += '<button class="layui-btn layui-btn-normal layui-btn-danger layui-btn-sm" data-open="' + init.push_all_url + '" data-title="全部发布"><i class="fa fa-plus"></i> 全部导入</button>\n';
                        }
                    }
                    else if (v === 'recycling') {
                        if (admin.checkAuth('recycling', elem)) {
                            toolbarHtml += '<button class="layui-btn layui-btn-sm layui-btn-danger" data-msg="确认要回收选中的数据吗?" data-url="' + init.recycling + '" data-table-delete="' + tableId + '"><i class="fa fa-trash-o"></i> 回收</button>\n';
            ...
            renderSelects: function (cols, tableInit, tableId, modifyReload) {
                tableInit.modify_url = tableInit.modify_url || false;
                cols = cols[0] || {};
                tableId = tableId || init.table_render_id;
                if (cols.length > 0) {
            ...
            buildToolbarHtml: function (toolbar, tableId) {
                ...
                else if(toolbar.method == 'tips')
                {
                    formatToolbar.method = formatToolbar.method !== '' ? 'data-tips="' + formatToolbar.url + '" data-title="' + formatToolbar.title + '" ' : '';
                }
                else
                {
            ...
            copy: function (data, option) {
                ...
                return '<a class="layuimini-table-url url-copy"  class="label bg-green " style="cursor:pointer">' + value + '</a>';
            },
            selects:function(data,option){
                var field = option.field;
                option.filter = option.filter || option.field || null;
                option.checked = option.checked || 1;
                option.tips = option.tips || '开|关';
                var list = option.selectList;
                ...
                var formHtml = '';
                var selectHtml = '';
                $.each(list, function (sI, sV) {
                    var selected = '';
                    if (sI === value) {
                        selected = 'selected=""';
                    }
                    selectHtml += '<option value="' + sI + '" ' + selected + '>' + sV + '</option>';
                });
                formHtml += '' +
                    '<select lay-uid="'+data.id+'" name="' + option.field + '" lay-submit lay-skin="selects" lay-filter="' + option.filter + 'caonima" >' +
                    selectHtml +
                    '</select>';

                return laytpl(formHtml).render(data);
            ...
            switch: function (data, option) {
                ...
                return laytpl('<input type="checkbox" data-id="'+data.id+'" name="' + option.field + '" value="' + data[field] + '" lay-skin="switch" lay-text="' + option.tips + '" lay-filter="' + option.filter + '" ' + checked + ' >').render(data);
            },
                ...
                form.on('submit(' + tableId + '_filter)', function (data) {
                    ...
                    if(data.form.action.indexOf("paylist/index.html") >0 ||  data.form.action.indexOf("payorder/index.html"))
                    {
                        admin.request.post({
                            ...
                            console.log(res)
                            $("#OrderCount").html(res.data[0].OrderCount)
                            $("#OrderMoney").html(res.data[0].OrderMoney)
                            $("#dayOrderCount").html(res.data[0].dayOrderCount)
                            $("#dayOrderMoney").html(res.data[0].dayOrderMoney)
                            $("#yesterdayOrderCount").html(res.data[0].yesterdayOrderCount)
                            $("#yesterdayOrderMoney").html(res.data[0].yesterdayOrderMoney)
                        })
                    }
            ...
            listenSelects:function (option, ok) {
                ...
                console.log('select(' + option.filter + 'caonima)');
                form.on('select(' + option.filter + 'caonima)', function (obj) {
                    console.log(obj);
                    var checked = obj.elem.selected;
                   var id =  $(obj.elem).attr('lay-uid');
            ...
            listenSwitch: function (option, ok) {
                ...
                form.on('switch(' + option.filter + ')', function (obj) {
                    ...
                    var data_id = $(obj.elem).attr('data-id');
                    if (typeof ok === 'function') {
                        return ok({
                            id: obj.value,
                            checked: checked,
        ...
        listen: function (preposeCallback, ok, no, ex) {
            ...
            $('body').on('click', '[data-open]', function () {
                ...
                if(checkbox === 'true'){
                    ...
                    if (ids.length <= 0) {
                        if(url == "stock/piliang")
                        {
                            admin.msg.error('请勾选需要操作的数据,已经发布的视频请勿勾选');
                            return false;
            ...
            $('body').on('click', '[data-tips]', function () {
            ...
            $('body').on('click', '[data-table-delete]', function () {
                ...
                admin.msg.confirm(msg, function () {
                    admin.request.post({
                        url: url,
                        data: {
                            id: ids
        ...
        api: {
            form: function (url, data, ok, no, ex, refreshTable,refreshFrame = false) {
                ...
                if (formList.length > 0) {
                    $.each(formList, function (i, v) {
                        var filter = $(this).attr('lay-filter'),
                            ...
                            refreshFrame = $(this).attr('data-refreshFrame'),
                        ...
                        form.on('submit(' + filter + ')', function (data) {
                            ...
                            admin.api.form(url, dataField, ok, no, ex, refresh,refreshFrame);
            ...
            editor: function () {
                ...
                if (editorList.length > 0) {
                    $.each(editorList, function (i, v) {
                       ...
                       var edito =  new editor(v);
                        edito.config.zIndex = 200;
                        edito.config.uploadImgServer = admin.url('ajax/uploadEditor')
                        edito.config.uploadImgAccept = ['jpg', 'jpeg', 'png', 'gif', 'bmp']
                        edito.config.uploadFileName = "upload";

                        var $text1 = $("#"+field)
                        edito.config.onchange = function (html) {
